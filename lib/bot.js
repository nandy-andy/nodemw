...
module.exports = (function() {
	...
	bot.prototype = {
		...
		getRecentChanges: function(start, end, callback) {
			function getMwTimestamp(date) {
			...
			// using default rcdir value 
			// which requires rcstart to be later than rcend
			// that's why we swap the value
			startDate = end ? new Date(end) : new Date(),
			endDate = start ? new Date(start) : false,
			params = {
				action: 'query',
                                list: 'recentchanges',
                                rcprop: props.join('|'),
			...
			startDate = getMwTimestamp(startDate);
			endDate = getMwTimestamp(endDate);

			if(end) {
				params.rcend = endDate;
			}

			params.rcstart = startDate;
			
			this.api.call(params, function(err, data, next) {
				callback && callback(err, ((data && data.recentchanges) || []), ((next && next.rcstart) || false));
			});
		},
